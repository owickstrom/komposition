...
insert focus insertion position parent = traversal
  >>= \t -> withParentOf t focus parent
 where
  traversal = case (insertion, position) of
    (InsertSequence sequence', RightOf) -> pure
      ( parentTraversal
        { onTimeline = \i (Timeline ann children') ->
          pure (Timeline ann (insertAtNonEmpty (succ i) sequence' children'))
        }
      )
    (InsertParallel parallel, RightOf) -> pure
      ( parentTraversal
        { onSequence = \i (Sequence ann children') ->
