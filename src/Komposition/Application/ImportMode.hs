...
selectFileToImport
  :: (Application t m sig, ImportEffects sig)
    ...
    fillForm model mf = do
      ...
      case (cmd, mf) of
        (CommandKeyMappedEvent Help, _) ->
          help #import [ModeKeyMap SImportMode (keymaps SImportMode)]
            >>>= \case
        ...
        (ImportDefaultVideoSpeedChanged s, form) -> fillForm
          model { setDefaultVideoSpeed = s }
          form { defaultVideoSpeed = s }
...
importSelectedFile gui existingProject' ImportFileForm { selectedFile, classify, defaultVideoSpeed }
  = do
...
getImportableFileMediaType
  :: (ImportEffects sig, Application t m sig)
  => FilePath
  -> t m r r (Maybe MediaType)
